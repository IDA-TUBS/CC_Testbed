...
# TODO Add changes to template file in IDADonkeyCar! 
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    V.add(DriveMode(cfg.AI_THROTTLE_MULT),
          inputs=['user/mode', 'user/angle', 'user/throttle', 'user/gear',
                  'pilot/angle', 'pilot/throttle', 'pilot/gear'],
          outputs=['steering', 'throttle', 'gear'])
...
class DriveMode:
    ...
    def run(self, mode,
            user_steering, user_throttle, user_gear,
            pilot_steering, pilot_throttle, pilot_gear):
        """
        ...
        """
        if mode == 'user':
            return user_steering, user_throttle, user_gear
        elif mode == 'local_angle':
            return pilot_steering if pilot_steering else 0.0, user_throttle, user_gear
        return (pilot_steering if pilot_steering else 0.0,
               pilot_throttle * self.ai_throttle_mult if pilot_throttle else 0.0,
               pilot_gear if pilot_gear else 0.0)
    ...
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:
        ...
        if cfg.CONTROLLER_TYPE == "pigpio_rc":  # an RC controllers read by GPIO pins. They typically don't have buttons
            ...
            V.add(
                ...
                outputs=['user/steering', 'user/throttle', 'user/gear',
                         'user/mode', 'recording'],
                threaded=True)
    return ctr
...
def add_drivetrain(V, cfg):
    ...
    if (not cfg.DONKEY_GYM) and cfg.DRIVE_TRAIN_TYPE != "MOCK":
        ...
        if cfg.DRIVE_TRAIN_TYPE == "PWM_STEERING_THROTTLE":
            ...
            #
            from donkeycar.parts.actuator import PWMSteering, PWMThrottle, PWMTransmission, PulseController
            ...
            gearbox_controller = PulseController(
                pwm_pin=pins.pwm_pin_by_id(dt["PWM_GEARBOX_PIN"]),
                pwm_scale=dt["PWM_GEARBOX_SCALE"],
                pwm_inverted=dt["PWM_GEARBOX_INVERTED"]) 
            gearbox = PWMTransmission(controller=gearbox_controller,
                                                first_gear=dt['GEARBOX_FIRST'],
                                                second_gear=dt['GEARBOX_SECOND'],
                                                third_gear=dt['GEARBOX_THIRD'])  
            V.add(steering, inputs=['steering'], threaded=True)
            V.add(throttle, inputs=['throttle'], threaded=True)
            V.add(gearbox, inputs=['gear'], threaded=True)
